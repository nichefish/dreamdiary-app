    ...
    <template v-else>
      ...
      <div class="card post mb-5">
        <div class="card-header">
          <h3 class="card-title">{{ t('user.signup.approval.pending.title') }}</h3>
        </div>
        <div class="card-body">
            ...
            <tbody>
              ...
              <tr v-for="req in store.pendingList" :key="'p-' + req.id">
                ...
                <td class="text-center">
                  <button type="button" class="btn btn-sm btn-primary me-2" @click="store.approve(req.id)">{{ t('user.list.action.approve') }}</button>
                  <button type="button" class="btn btn-sm btn-light-danger" @click="store.reject(req.id)">{{ t('user.list.action.reject') }}</button>
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div class="card post">
        <div class="card-header">
          <h3 class="card-title">{{ t('user.signup.approval.recent.title') }}</h3>
        </div>
        <div class="card-body">
          <table class="table align-middle table-row-dashed fs-small gy-3 table-fixed mb-0">
            <thead>
