package io.nicheblog.dreamdiary.feature.journal.day.mapstruct;

import io.nicheblog.dreamdiary.feature.journal.day.model.JournalDayCalDto;
import io.nicheblog.dreamdiary.feature.journal.day.model.JournalDayDto;
import org.mapstruct.*;
import org.mapstruct.factory.Mappers;

/**
 * JournalDayCalMapstruct
 * <pre>
 *  저널 일자 달력 MapStruct 기반 Mapper 인터페이스.
 * </pre>
 *
 * @author nichefish
 */
@Mapper(componentModel = "spring", unmappedTargetPolicy = ReportingPolicy.IGNORE, builder = @Builder(disableBuilder = true))
public interface JournalDayCalMapstruct {

    JournalDayCalMapstruct INSTANCE = Mappers.getMapper(JournalDayCalMapstruct.class);

    /**
     * Dto -> CalDto 변환
     *
     * @param dto 변환할 Entity 객체
     * @return CalDto -- 변환된 달력 Dto 객체
     */
    @Named("toCalDto")
    @Mapping(target = "start", source = "stdrdDt")
    @Mapping(target = "end", source = "stdrdDt")
    JournalDayCalDto toCalDto(final JournalDayDto dto) throws Exception;
}
