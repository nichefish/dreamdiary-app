...
public class BoardPostRestController
    ...
    /**
     * 게시판별 활성 말머리 목록 조회 (Ajax).
     * <p>
     * boardKey로 식별되는 GLOBAL Prefix Scope의 활성 Prefix만 반환한다.
     * 목록이 비어 있으면 화면은 말머리 select를 숨긴다.
     *
     * @param boardKey 게시판 키
     * @return {@link ResponseEntity} -- 활성 말머리 목록 (없으면 빈 목록)
     */
    @GetMapping(Url.BOARD_PREFIXES)
    @Secured({Constant.ROLE_USER, Constant.ROLE_MNGR})
    @ResponseBody
    public ResponseEntity<AjaxResponse> boardPrefixListAjax(
            final @PathVariable("boardKey") String boardKey
    ) {
        final List<PrefixDto> prefixList = boardPrefixService.getActive(boardKey);
        log.debug("[boardPrefixListAjax] 활성 말머리 조회. boardKey={}, size={}", boardKey, prefixList.size());
     ...
     *
     * @param searchParam 검색 조건 (contentType, prefixId, searchKeyword 등)
