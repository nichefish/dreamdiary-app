/**
 * journal_sumry_module.ts
 * 저널 결산 스크립트 모듈
 *
 * @author nichefish
 */
if (typeof dF === 'undefined') { var dF = {} as any; }
dF.JournalSumry = (function(): dfModule {
    return {
        initialized: false,
         ...
         */
        init: function(): void {
            if (dF.JournalSumry.initialized) return;

            dF.JournalSumry.initialized = true;
            console.log("'dF.JournalSumry' module initialized.");
        },
        ...
        initForm: function(obj: Record<string, any> = {}): void {
            /* show modal */
            cF.handlebars.modal(obj, "journal_sumry_reg", ["header"]);

            /* jquery validation */
            cF.validate.validateForm("#journalSumryRegForm", dF.JournalSumry.regAjax);
            /* tagify */
        ...
        listAjax: function(): void {
            ...
            cF.ajax.get(url, null, function(res: AjaxResponse): void {
                ...
                const { rsltList } = res;
                cF.ui.closeModal();
                cF.handlebars.template(rsltList, "journal_sumry_list");
        ...
        dtlView: function(yy: string|number): void {
            ...
            location.href = cF.util.bindUrl(Url.JOURNAL_SUMRY_VIEW, {yy}) + "?section=DIARY";
        },
        ...
        dtlViewWithSection: function(section: "DIARY"|"DREAM"): void {
            ...
            location.href = cF.util.bindUrl(Url.JOURNAL_SUMRY_VIEW, {yy}) + `?section=${section}`;
        },
        ...
        dtlAjax: function(yy: string|number): void {
            const url: string = cF.util.bindUrl(Url.JOURNAL_SUMRY, { yy });
            cF.ajax.get(url, null, function(res: AjaxResponse): void {
                if (!res.rslt) {
                    if (cF.util.isNotEmpty(res.message)) Swal.fire({ text: res.message });
                    return;
                ...
                /* show modal */
                cF.handlebars.template(rsltObj, "journal_sumry_dtl");
            });
        },
        ...
        toggleParam: function(): void {
            ...
            switch (section) {
                case "DIARY":
                    ...
                    break;
                case "DREAM":
                    dF.JournalSumry.getSumryDreamListAjax(yy);
                    break;
            }
        },
        ...
        getSumryDiaryListAjax: function(yy: string|number): void {
            ...
            if (!showImprtc && !showRefrnc) {
                cF.handlebars.template([], "journal_sumry_diary_list");
                return;
            }

            const url: string = cF.util.bindUrl(Url.JOURNAL_SUMRY_DIARIES, { yy });
        ...
        getSumryDreamListAjax: function(yy: string|number): void {
            const url: string = cF.util.bindUrl(Url.JOURNAL_SUMRY_DREAMS, { yy });
            cF.ajax.get(url, null, function(res: AjaxResponse): void {
                if (!res.rslt) {
                    if (cF.util.isNotEmpty(res.message)) Swal.fire({ text: res.message });
                    return;
        ...
        getTagListAjax: function(yy: string|number, type: "DAY"|"DIARY"|"DREAM"): void {
            const url: string = cF.util.bindUrl(Url.JOURNAL_SUMRY_TAGS, { yy }) + `?type=${type}`;
            cF.ajax.get(url, null, function(res: AjaxResponse): void {
                if (!res.rslt) {
                    if (cF.util.isNotEmpty(res.message)) Swal.fire({ text: res.message });
                    return;
                ...
                cF.handlebars.template(rsltList, `journal_sumry_${lowerType}_tag_list`);
            });
        },
         ...
         */
        list: function(): void {
            cF.ui.blockUIReplace(Url.JOURNAL_SUMRY_LIST);
        },
        ...
        makeYySumryAjax: function(yy: string|number): void {
            ...
            const url: string = Url.JOURNAL_SUMRY_MAKE_AJAX;
        ...
        makeTotalSumryAjax: function(): void {
            const url: string = Url.JOURNAL_SUMRY_MAKE_TOTAL_AJAX;
        ...
        comptAjax: function(id: string|number): void {
            ...
            const url: string = Url.JOURNAL_SUMRY_DREAM_COMPT_AJAX;
        ...
        submit: function(): void {
            tinymce.get("tinymce_journalSumryCn").save();
            $("#journalSumryRegForm").submit();
        },
        ...
        mdfModal: function(yy: string|number): void {
            ...
            cF.ajax.get(url, null, function(res: AjaxResponse): void {
                ...
                const { rsltObj } = res;
                /* initialize form. */
                dF.JournalSumry.initForm(rsltObj);
            });
        },
        ...
        regAjax: function(): void {
            Swal.fire({
                ...
                const url: string = Url.JOURNAL_SUMRY_REG_AJAX;
                const ajaxData: FormData = new FormData(document.getElementById("journalSumryRegForm") as HTMLFormElement);
