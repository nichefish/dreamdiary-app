...
/**
 * journal_day_meta_module.ts
 * 저널 일자 태그 스크립트 모듈
 *
 * @author nichefish
 */
if (typeof dF === 'undefined') { var dF = {} as any; }
dF.JournalDayMeta = (function(): dfModule {
    return {
        initialized: false,
        ctgrMap: new Map(),
         ...
         */
        init: function(): void {
            if (dF.JournalDayMeta.initialized) return;

            dF.JournalDayMeta.getCtgrMap();
            ...
            document.addEventListener('click', function (e: MouseEvent): void {
                ...
                dF.JournalDayMeta.modal(metaId);
            });

            dF.JournalDayMeta.initialized = true;
            console.log("'dF.JournalDayMeta' module initialized.");
        },
        ...
        getCtgrMap: function(): void {
            const url: string = Url.JOURNAL_DAY_META_CTGR_MAP;
            cF.ajax.get(url, {}, function(res: AjaxResponse): void {
                if (res.rsltMap) dF.JournalDayMeta.ctgrMap = res.rsltMap;
            });
        },
        ...
        listAjax: function(): void {
            const url: string = Url.JOURNAL_DAY_METAS;
            cF.ajax.get(url, null, function(res: AjaxResponse): void {
                if (!res.rslt) {
                    if (cF.util.isNotEmpty(res.message)) Swal.fire({ text: res.message });
                    return;
                }
                cF.handlebars.template(res.rsltList, "journal_day_meta_list");
            });
        },
        ...
        modal: function(metaId: string|number, yy?: string|number): void {
            ...
            const preferredYy: string = dF.JournalDayMeta.getSelectedYy(yy);
            dF.JournalDayMeta.getYyListAjax(metaId, function(yyList: any[]): void {
                const selectedYy: string = dF.JournalDayMeta.normalizeSelectedYy(preferredYy, yyList);
                ...
                cF.ajax.get(url, ajaxData, function(res: AjaxResponse): void {
                    ...
                    cF.handlebars.modal({
                        metaId,
                        yy: selectedYy,
                        yearOptions: dF.JournalDayMeta.getYearOptions(selectedYy, yyList),
                        list: res.rsltList
                    }, "journal_day_meta");
        ...
        changeYy: function(metaId: string|number, yy: string|number): void {
            if (isNaN(Number(metaId))) return;
            dF.JournalDayMeta.modal(metaId, yy);
        },
        ...
        select: function(metaId: string|number): void {
            ...
            cF.ajax.get(url, null, function(res: AjaxResponse): void {
                ...
                cF.handlebars.template(res.rsltObj, "journal_day_meta_config");
            });
        }
    }
})();
