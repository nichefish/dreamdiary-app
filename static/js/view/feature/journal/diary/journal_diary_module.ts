...
dF.JournalDiary = (function(): dfModule {
    return {
         ...
         */
        regModal: function({ journalDayId, journalChapterId, stdrdDt, journalDateWeekDay }: { journalDayId: string | number; journalChapterId: string | number; stdrdDt: string; journalDateWeekDay: string; }): void {
            ...
            cF.ajax.get(url, null, function(res: AjaxResponse): void {
                ...
                const obj: Record<string, any> = { journalDayId: journalDayId, journalChapterId: journalChapterId, stdrdDt: stdrdDt, journalDateWeekDay: journalDateWeekDay, chapterList: chapterList };
                /* initialize form. */
        ...
        toggleStateAjax: function(id: string|number, stateKey: string, { onOffFunc }): void {
            ...
            const payload = { id, contentType: "JOURNAL_DIARY", stateKey, cacheContext };
            dF.State.toggleAjax(payload, function(res: AjaxResponse): void {
                ...
                const chk: HTMLInputElement = item.querySelector(`.diary-context-${lowerStateKey}-check`);
        ...
        buildViewModel: function(diary, profileName) {
            ...
            const hasState = (targetState: string): boolean =>
                Array.isArray(diary.state?.list) && diary.state.list.some((state: any): boolean => state?.stateKey === targetState);
