/**
 * journal_diary_tag_module.ts
 * 저널 일기 태그 스크립트 모듈
 * 
 * @author nichefish
 */
if (typeof dF === 'undefined') { var dF = {} as any; }
dF.JournalDiaryTag = (function(): dfModule {
    return {
        initialized: false,
        initPromise: null,
        ctgrMap: new Map(),
        ...
        init: async function(): Promise<void> {
            ...
            this.initPromise = (async () => {
                await dF.JournalDiaryTag.getCtgrMap();
                await dF.JournalDiaryTag.getNmList();
                this.initialized = true;
                console.log("'dF.JournalDiaryTag' module initialized.");
            })();
        ...
        getCtgrMap: async function(): Promise<void> {
            ...
            return cF.ajax.get(url, {}, function(res: AjaxResponse): void {
                if (res.rsltMap) dF.JournalDiaryTag.ctgrMap = res.rsltMap;
            });
        },
        ...
        getNmList: async function(): Promise<void> {
            ...
            return cF.ajax.get(url, {}, function(res: AjaxResponse): void {
                if (res.rsltList) dF.JournalDiaryTag.list = res.rsltList;
            });
        },
        ...
        listAjax: function(): void {
            ...
            if (dF.JournalDay?.viewType === "WEEKLY") {
                const weekStartDt: string = dF.JournalDiaryTag.getCurrentWeekStartDt();
            ...
            cF.ajax.get(url, ajaxData, function(res: AjaxResponse): void {
                ...
                cF.handlebars.template(res.rsltList, "journal_diary_tag_list");
            });
        },
        ...
        openSearch: function(tagId: string|number): void {
            let url: string = `${Url.JOURNAL_DIARY_SEARCH}?tagIds=${tagId}`;
            if (dF.JournalDay?.viewType === "WEEKLY") {
                const weekStartDt: string = dF.JournalDiaryTag.getCurrentWeekStartDt();
        ...
        select: function(tagId: string|number, tagNm?: string, ctgr: string = ""): void {
            if (dF.JournalDayTag?.isContextMenuEnabled?.()) {
                dF.JournalDayTag.openContextMenu(tagId, tagNm ?? "", ctgr, function(): void {
                    dF.JournalDiaryTag.openSearch(tagId);
                }, "JOURNAL_DIARY");
                return;
            }
