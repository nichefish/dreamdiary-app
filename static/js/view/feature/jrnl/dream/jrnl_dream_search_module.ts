...
dF.JrnlDreamSearch = (function(): dfModule {
    return {
         ...
         */
        init: function(): void {
            if (dF.JrnlDreamSearch.initialized) return;

            dF.JrnlDreamSearch.initialized = true;
            console.log("'dF.JrnlDreamSearch' module initialized.");
        },
        ...
        initKeyword: function(): void {
            dF.JrnlDreamSearch.clearKeywordFields();

            const params: URLSearchParams = new URLSearchParams(window.location.search);
            const keywords: string[] = params.getAll("searchKeywords");
            if (keywords.length > 0) {
                keywords.forEach((k: string) => dF.JrnlDreamSearch.addKeyword(k));
            } else {
        ...
        initTag: function(): void {
            ...
            if (tagIds.length > 0) {
                tagIds.forEach((tagId: string): void => {
                    const tag = dF.JrnlDreamTag.list.find(
                        (t: any): boolean => t.id === Number(tagId)
                    );
                    if (!tag) return;
                    dF.JrnlDreamSearch.select(tagId, tag.tagNm);
                });
            }
        },
        ...
        addKeyword: function(value?: string): void {
            ...
            statusBadge.innerHTML = `
                ${value}
                <i class="bi bi-x cursor-pointer" onclick="dF.JrnlDreamSearch.removeKeyword('${value}')"></i>
            `;
        ...
        select: function(tagId: string|number, tagNm: string): void {
            ...
            tagBadge.innerHTML = `
                #${tagNm}
                <i class="bi bi-x cursor-pointer" onclick="dF.JrnlDreamSearch.removeTag('${tagId}')"></i>
            `;
