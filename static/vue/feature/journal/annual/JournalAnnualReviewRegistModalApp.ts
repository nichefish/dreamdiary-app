/**
 * JournalAnnualReviewRegistModalApp.ts
 * 저널 결산 리뷰 등록/수정 모달(`journal_annual_review_regist`) — Handlebars 헤더/본문 템플릿 대체.
 *
 * 변경(A-3):
 *   - `_journal_annual_review_regist_modal_header_template.hbs` / `_journal_annual_review_regist_modal_template.hbs` (tagify partial 한 줄) 본문 렌더와
 *     `dF.JournalAnnualReview.initForm` 의 `cF.handlebars.modal` 진입을 본 모듈로 단일 수렴한다.
 *   - 저장 버튼은 `window.JournalAnnualReviewRegistVueApp.submit()` 가 `dF.JournalAnnualReview.submit()` 로 위임한다.
 *   - jQuery validate / TinyMCE 누적 방지를 위해 open 시점에 destroy → init 순서로 부착한다.
 *   - tagify 호출 시그니처는 *행위 보존*을 위해 원본 initForm 의 두 호출을 그대로 옮긴다:
 *       (1) `cF.tagify.initWithCtgr("#journalAnnualReviewRegistForm #tagListStr", undefined)`
 *       (2) `dF.JournalAnnualReview.tagify = cF.tagify.init("#journalAnnualReviewRegistForm #tagListStr")`
 ...
 *   - 본 모달은 `journal_annual_detail.ftlh` 의 `_journal_annual_review_regist_modal.ftlh` include 묶음으로만 진입한다
 *     (list 페이지에서는 review 진입점 0 — include 자체가 dtl 전용).
 *
 * @author nichefish
 */

import JournalAnnualReviewRegistModalHeader from "./components/JournalAnnualReviewRegistModalHeader.js";
import JournalAnnualReviewRegistModalBody from "./components/JournalAnnualReviewRegistModalBody.js";

type JournalAnnualReviewRegistVueBridge = {
    mounted?: boolean;
...
const VUE_MOUNT_ID = "journal_annual_review_regist_vue_app";
/** Vue 가 헤더 마크업을 teleport 할 대상 div id. */
const HEADER_TELEPORT_ID = "journal_annual_review_regist_modal_header_div";
/** Vue 가 본문(tagify) 마크업을 teleport 할 대상 div id. */
const BODY_TELEPORT_ID = "journal_annual_review_regist_div";
/** form id. */
const FORM_SELECTOR = "#journalAnnualReviewRegistForm";
/** TinyMCE editor id (textarea id 와 동일). */
const TINYMCE_ID = "tinymce_journalAnnualReviewCn";
/** Bootstrap 모달 element id (modal_layout id `journal_annual_review_regist` + `_modal`). */
const MODAL_EL_ID = "journal_annual_review_regist_modal";
/** tagify selector — initForm 원본과 동일. */
const TAGIFY_SELECTOR = "#journalAnnualReviewRegistForm #tagListStr";
...
function destroyPreviousValidator(): void {
    ...
    if (validator && typeof validator.destroy === "function") {
        try {
            ...
            console.warn("[JournalAnnualReviewRegistModalApp] jQuery validate destroy failed", e);
        }
    }
...
function destroyPreviousTinymce(): void {
    try {
        ...
        console.warn("[JournalAnnualReviewRegistModalApp] tinymce destroy failed", e);
    }
}
...
function attachRegistFormControls(model: Record<string, any>): void {
    ...
    if (!module) {
        console.error("[JournalAnnualReviewRegistModalApp] dF.JournalAnnualReview missing.");
        return;
    }
...
function showModal(): void {
    ...
    if (!modalEl) {
        console.error("[JournalAnnualReviewRegistModalApp] Modal element not found:", MODAL_EL_ID);
        return;
    }
...
function submitBridge(): void {
    ...
    if (!mod || typeof mod.submit !== "function") {
        console.error("[JournalAnnualReviewRegistModalApp] submit — dF.JournalAnnualReview.submit unavailable.");
        return;
    }
    mod.submit();
}
...
function createRootComponent(): Record<string, unknown> {
    return {
        name: "JournalAnnualReviewRegistRoot",
        components: { JournalAnnualReviewRegistModalHeader, JournalAnnualReviewRegistModalBody },
        ...
        <teleport :to="headerTo">
            <JournalAnnualReviewRegistModalHeader v-if="state.model" :model="state.model" />
        </teleport>
        <teleport :to="bodyTo">
            <JournalAnnualReviewRegistModalBody v-if="state.model" :model="state.model" />
        </teleport>
        `,
    };
}
...
runWhenDomReady(function(): void {
    const priorBridge = ((window as any).JournalAnnualReviewRegistVueApp ?? {}) as JournalAnnualReviewRegistVueBridge;
    ...
    if (!mountEl) {
        ...
        console.log("[JournalAnnualReviewRegistModalApp] mount element not found:", VUE_MOUNT_ID);
        return;
    }
    ...
    (window as any).JournalAnnualReviewRegistVueApp = {
        mounted: true,
        pendingPayload: null,
        open: function(model: Record<string, any>): void {
            ...
            const b = (window as any).JournalAnnualReviewRegistVueApp as JournalAnnualReviewRegistVueBridge;
            b.pendingPayload = model;
            console.log("[JournalAnnualReviewRegistModalApp] pending payload queued.");
        },
        submit: submitBridge,
    };
