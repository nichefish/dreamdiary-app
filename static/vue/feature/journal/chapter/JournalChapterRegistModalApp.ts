/**
 * JournalChapterRegistModalApp.ts
 * 저널 챕터 등록/수정 모달(`journal_chapter_regist`) 본문 렌더 — Handlebars 템플릿 대체.
 *
 * 변경(Phase B):
 *   - _journal_chapter_regist_modal_template.hbs(HBS) 본문 렌더와
 *     dF.JournalChapter 의 registModal/modifyModal/initForm/submit/registAjax 진입을 이 모듈로 단일 수렴한다.
 *   - 기존 외부 호출(`dF.JournalChapter.registModal({...})` / `dF.JournalChapter.modifyModal(id, ...)`)은
 *     `window.JournalChapterRegistVueApp.open(model)` / `window.JournalChapterRegistVueApp.submit()` 큐로 대체된다.
 *   - registAjax 후속 처리(저장 후 목록 갱신·태그 패널 갱신)는 기존 dF.JournalChapter.registAjax 의 분기를 그대로 옮긴다.
...
import JournalChapterRegistModalBody from "./components/JournalChapterRegistModalBody.js";
// 변경(D): `Message.get` 직호출을 `resolveMessage` 헬퍼로 위임.
import { resolveMessage } from "../../../common/messageHelper.js";

type JournalChapterRegistVueBridge = {
    mounted?: boolean;
...
/**
 * FTL 부트스트랩(`__journalChapterRegistBootstrap.categoryOptions`) 에서 카테고리 옵션을 읽어온다.
 * - SSOT: 서버 모델 `JOURNAL_CHAPTER_CTGR_CD` (FTL `_journal_chapter_regist_modal.ftlh` 인라인 적재).
...
function readCategoryOptions(): Array<{ code: string; codeName: string }> {
    const raw = (window as any).__journalChapterRegistBootstrap?.categoryOptions;
...
function showRegistModal(): void {
    const modalEl = document.querySelector("#journal_chapter_regist_modal") as HTMLElement | null;
    if (!modalEl) {
        console.error("[JournalChapterRegistModalApp] Modal root #journal_chapter_regist_modal not found.");
        return;
    }
...
function destroyPreviousValidator(): void {
    const $form = $("#journalChapterRegistForm");
    const validator = $form.data("validator") as { destroy?: () => void } | undefined;
    if (validator && typeof validator.destroy === "function") {
        try {
            validator.destroy();
        } catch (e) {
            console.warn("[JournalChapterRegistModalApp] jQuery validate destroy failed", e);
        }
    }
...
/**
 * 등록/수정 처리 (Ajax).
 * 변경 전: dF.JournalChapter.registAjax — JournalDayCalVueApp.refresh / journalDayResolveListBridge().refresh 직접 호출.
...
function registAjax(): void {
    const id: string = cF.util.getInputValue("#journalChapterRegistForm [name='id']");
    ...
    Swal.fire({
        ...
        const url: string = isModify ? cF.util.bindUrl(Url.JOURNAL_CHAPTER, { id }) : Url.JOURNAL_CHAPTERS;
        const ajaxData: FormData = new FormData(document.getElementById("journalChapterRegistForm") as HTMLFormElement);
        cF.$ajax.multipart(url, ajaxData, function(res: AjaxResponse): void {
            Swal.fire({ text: res.message })
                .then(function(): void {
                    if (!res.rslt) return;

                    /* 변경 전: dF.JournalChapter.registAjax 안에서 동일 분기 처리.
 ...
 * 변경 전: dF.JournalChapter.initForm — `cF.validate.validateForm("#journalChapterRegistForm", dF.JournalChapter.registAjax)` 호출.
 * 변경 후(Phase B): Vue 모달 측 registAjax 를 직접 검증 콜백으로 부착한다.
 */
function attachRegistFormControls(): void {
    destroyPreviousValidator();
    cF.validate.validateForm("#journalChapterRegistForm", registAjax);
}

/**
 * 저장 버튼(footer) 트리거. 폼 submit → jQuery validate → registAjax 흐름 진입.
 */
function submit(): void {
    $("#journalChapterRegistForm").submit();
}

/**
 * 모달 오픈 — 외부에서 `window.JournalChapterRegistVueApp.open(model)` 단일 진입.
...
function openRegist(model: Record<string, any>): void {
    ...
    Vue.nextTick(function(): void {
        attachRegistFormControls();
        showRegistModal();
    });
}

const JournalChapterRegistRootApp = {
    name: "JournalChapterRegistRootApp",
    components: {
        JournalChapterRegistModalBody,
    },
    data(): { state: typeof state } {
        return { state };
    },
    template: `
    <teleport to="#journal_chapter_regist_div">
        <JournalChapterRegistModalBody v-if="state.model" :model="state.model" />
    </teleport>
    `,
};
...
runWhenDomReady(async function(): Promise<void> {
    const mountEl = document.querySelector("#journal_chapter_regist_vue_app") as HTMLElement | null;
    if (!mountEl) {
        console.error("[JournalChapterRegistModalApp] Mount root #journal_chapter_regist_vue_app not found.");
        return;
    }

    await i18n.load(resolveLocale());

    const priorBridge = (window.JournalChapterRegistVueApp ?? {}) as JournalChapterRegistVueBridge;
    const pendingPayload: Record<string, any> | null | undefined = priorBridge.pendingPayload;

    const app = Vue.createApp(JournalChapterRegistRootApp);
    app.config.globalProperties.$t = (key: string): string => t(key);
    app.mount("#journal_chapter_regist_vue_app");

    window.JournalChapterRegistVueApp = {
        mounted: true,
        pendingPayload: null,
        open: openRegist,
        submit: submit,
