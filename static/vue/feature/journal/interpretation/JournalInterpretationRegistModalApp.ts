/**
 * JournalInterpretationRegistModalApp.ts
 * 저널 해석 등록/수정 모달(`journal_interpretation_regist`) — Handlebars 헤더/본문 템플릿 대체.
 *
 * 변경(I-2):
 *   - _journal_interpretation_regist_modal_header_template.hbs / _journal_interpretation_regist_modal_template.hbs(빈 body) 본문 렌더와
 *     dF.JournalInterpretation.initForm 의 cF.handlebars.modal 진입을 이 모듈로 단일 수렴한다.
 *   - 저장 버튼은 `window.JournalInterpretationRegistVueApp.submit()` 가 `dF.JournalInterpretation.submit()` 로 위임한다.
 * 변경(I-3):
 *   - `submit` 구현은 `journalInterpretationCrudService.submit` 로 추출되었다. 브리지 체인은 동일하다(RegistVueApp → dF.JournalInterpretation.submit → CrudService).
 *   - 등록/수정 확인(registAjax)·목록 갱신 로직도 동일한 service(`journalInterpretationCrudService.registAjax`) 로 이전되었다.
 ...
 *     본 모달 진입의 브리지 체인은 동일(RegistVueApp → dF.JournalInterpretation.<method> → State/Crud service).
 *
 * @author nichefish
 */

import { createScopedI18n } from "../../../global/services/scopedI18nService.js";
import JournalInterpretationRegistModalHeader from "./components/JournalInterpretationRegistModalHeader.js";

type JournalInterpretationRegistVueBridge = {
    mounted?: boolean;
...
const VUE_MOUNT_ID = "journal_interpretation_regist_vue_app";
/** Vue 가 헤더 마크업을 teleport 할 대상 div id. */
const HEADER_TELEPORT_ID = "journal_interpretation_regist_modal_header_div";
/** form id. */
const FORM_SELECTOR = "#journalInterpretationRegistForm";
/** TinyMCE editor id (textarea id 와 동일). */
const TINYMCE_ID = "tinymce_journalInterpretationCn";
/** Bootstrap 모달 element id (hbs `journal_interpretation_regist_modal`). */
const MODAL_EL_ID = "journal_interpretation_regist_modal";
/** imprtcYn 체크박스 라벨 — 기존 initForm 동작 보존. 폼에 #imprtcYn element 가 없으면 noop. */
const IMPRTC_LABEL_SELECTOR = "#journalInterpretationRegistForm #imprtcYn";
...
function destroyPreviousValidator(): void {
    ...
    if (validator && typeof validator.destroy === "function") {
        try {
            ...
            console.warn("[JournalInterpretationRegistModalApp] jQuery validate destroy failed", e);
        }
    }
...
function destroyPreviousTinymce(): void {
    try {
        ...
        console.warn("[JournalInterpretationRegistModalApp] tinymce destroy failed", e);
    }
}
...
function attachRegistFormControls(model: Record<string, any>): void {
    ...
    if (!module) {
        console.error("[JournalInterpretationRegistModalApp] dF.JournalInterpretation missing.");
        return;
    }
...
function showModal(): void {
    ...
    if (!modalEl) {
        console.error("[JournalInterpretationRegistModalApp] Modal element not found:", MODAL_EL_ID);
        return;
    }
...
function submitBridge(): void {
    ...
    if (!mod || typeof mod.submit !== "function") {
        console.error("[JournalInterpretationRegistModalApp] submit — dF.JournalInterpretation.submit unavailable.");
        return;
    }
    mod.submit();
}
...
function createRootComponent(): Record<string, unknown> {
    return {
        name: "JournalInterpretationRegistRoot",
        components: { JournalInterpretationRegistModalHeader },
        ...
        <teleport :to="headerTo">
            <JournalInterpretationRegistModalHeader v-if="state.model" :model="state.model" />
        </teleport>
        `,
    };
}
...
runWhenDomReady(async function(): Promise<void> {
    ...
    const priorBridge = (window.JournalInterpretationRegistVueApp ?? {}) as JournalInterpretationRegistVueBridge;
    ...
    if (!mountEl) {
        ...
        console.log("[JournalInterpretationRegistModalApp] mount element not found:", VUE_MOUNT_ID);
        return;
    }
    ...
    window.JournalInterpretationRegistVueApp = {
        mounted: true,
        pendingPayload: null,
        open: function(model: Record<string, any>): void {
            ...
            const b = window.JournalInterpretationRegistVueApp as JournalInterpretationRegistVueBridge;
            b.pendingPayload = model;
            console.log("[JournalInterpretationRegistModalApp] pending payload queued.");
        },
        submit: submitBridge,
    };
