...
- **스레드-엔트리 소속 계약 (`journal_thread_entry`)**
  ...
  - `sort_order` 는 스키마에 nullable 로 남아 있지만 **현재 표시 순서 계약에는 사용하지 않는다**. 스레드 상세의 선후는 엔트리 일자 오름차순, 동일 일자는 엔트리 ID 오름차순으로 결정한다. 구체적인 사용 사례 없이 사용자 지정 순서 UI/API를 연결하지 않는다.
...
- **스레드 내 위치·역할의 현재 결정**
  ...
  - 읽기 순서는 엔트리 일자에서 자연스럽게 파생한다. 현재 계약은 일자 오름차순, 동일 일자는 엔트리 ID 오름차순이며 수동 순서를 제공하지 않는다.
