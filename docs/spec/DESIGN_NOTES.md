...
- **결정 변경 (0.24, 스레드 소속 구현) — FLOW 를 스레드 소속으로 수렴**
  ...
  - 단계:
    ...
    2. **완료** — FLOW 간선 → 소속 이관 (`V0.24.8`). 간선 29건 / 엔트리 33개 / 연결 컴포넌트 4개를 스레드 4건 + 소속 33건으로 옮겼다. 네 컴포넌트 모두 성형(star) 구조였다(허브 차수 합 26/29) — 사용자가 이미 «간선»이 아니라 «소속»으로 쓰고 있었다는 근거다. `related_content` 의 FLOW 29행은 롤백 여지를 위해 **남겨 둔다**.
    3. 예정 — ⋯ 메뉴의 FLOW 항목을 소속 UI 로 교체
    4. 예정 — `related_content` 의 FLOW 관련 코드·데이터 제거
...
- `FLOW`는 두 엔트리가 같은 전개 흐름에 있다는 사실을 나타내는 **무방향 관계**다. 관계 자체에 출발점·도착점이나 인과 방향을 저장하지 않는다.
- 일반 관련글은 기준 엔트리와 직접 연결된 한 단계 관계를 보여준다. `FLOW`만 전이적으로 탐색하여 연결 컴포넌트 전체를 종단 보기의 범위로 사용한다.
- 종단 보기의 선후관계는 관계 행이 아니라 엔트리 자체의 시간·표시 순서에서 파생한다. 기본 정렬 키는 `stdrdDt → journalChapter.sortOrder → 콘텐츠 유형 표시 순서 → entry.sortOrder → entry.id`다.
- FLOW 연결을 추가해 두 연결 컴포넌트가 맞닿으면 하나의 흐름으로 합쳐진다. 연결을 해제해 경로가 끊기면 다시 여러 흐름으로 분리된다. 별도 그룹·이슈·스레드 행을 생성하지 않는다.
- 저널 스레드는 독립적인 상위 서사와 엔트리 소속을 관리한다. FLOW는 임의의 엔트리를 앵커로 관련 엔트리를 종단 탐색하는 축이며 스레드의 대체 구현으로 사용하지 않는다.
- `CAUSE`처럼 방향을 암시하는 기존 관계 타입은 무방향 모델에서는 「어느 쪽이 원인인가」를 표현하지 않는다. 유지할 경우 사용자 의미는 「인과 관계가 있음」처럼 대칭적인 라벨로 제한한다.

#### FLOW 구현 TODO

- [ ] 현재 `JOURNAL_DIARY`·`JOURNAL_DREAM`으로 제한된 RELATED 지원 범위를 노트·해석까지 확장할지 확정한다.
- [ ] 같은 날짜의 챕터·콘텐츠 유형 간 표시 순서와 서버 정렬 키를 단일 계약으로 확정한다.
- [ ] 순환 그래프를 중복 없이 탐색하고 과도한 연결 컴포넌트에 상한 또는 페이지 정책을 적용한다.
- [ ] FLOW 연결로 두 기존 흐름이 합쳐질 때 사용자에게 병합 결과를 안내한다.
- [x] 엔트리에서 FLOW 연결·해제와 종단 보기에 진입하는 사용자 문구를 확정한다. (`흐름 연결` / `연결 해제` / `흐름 보기`)
- [x] 일반 RELATED 한 단계 조회와 FLOW 전이 조회를 분리한 API·DTO 계약을 정의한다. (`GET /api/related/{contentType}/{id}/flow`)
