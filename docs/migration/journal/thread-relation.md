...
- **OFF**: 그 스레드(base)의 멤버 엔트리만 시간축에.
- **ON**: base + 직접 연관 스레드의 엔트리를 한 시간축에 날짜순으로 섞어 본다.
...
3. **토글 상태 = 화면 임시.** 기본 OFF, 저장하지 않는다(뷰 옵션 성격).
...
- 스레드 상세 조회에 includeRelated(또는 동등) 옵션을 둔다. ON 이면 **base + 직접 연관 스레드의 멤버 엔트리**를 합쳐(중복 제거) 기존 정렬축(일자 → 원본 엔트리 sort_order → ID)으로 내려준다.
...
- 프론트: 스레드 상세에 연관 스레드 추가·검색(스레드 피커)과 연관 목록 표시, 뷰 합성 토글.
