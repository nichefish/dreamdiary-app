...
- 시스템 요약은 `summaryYn=Y`로 판정하고 고정 `요약` 명칭을 표시하며 Prefix 선택기를 제공하지 않는다. DREAM도 Prefix를 허용하지 않는다. 일자 내 챕터 순서는 정규화·조회 모두 **요약 맨 앞 → 일반 → DREAM 맨 뒤**이며, 일반 챕터 `#1`은 요약 바로 다음이다.
