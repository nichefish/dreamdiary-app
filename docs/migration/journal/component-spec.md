...
### 22-4c. `JournalEntryViewPage` 저장 항목 읽기 전용 새 창

**Vue**: `JournalEntryViewPage.vue` → route `/journal/entry/view-pop?entryId={id}` (`journal-entry-view-popup`, `SystemLayout`)

엔트리·임베드 Reflection ⋯ 메뉴의 `common.open-in-new-window`가 여는 저장 항목 전용 화면이다. `GET /api/journal/entry/{id}`가 실제 contentType과 현재 사용자 소유권을 판별해 Primary 엔트리·Reflection을 같은 경로로 조회한다. 날짜·유형·순번·말머리·제목·꿈꾼(해당 시)·`markdownContent`·태그(해당 시)를 읽기 전용으로 표시하며 편집·상태 변경·모달 액션은 제공하지 않는다. `openJournalEntryViewPopup`은 `journal_entry_view_{id}` 창 이름을 사용해 같은 ID 창을 재사용하고 다른 ID 창은 분리한다. popup route는 `AppChat`을 숨기고 세션 만료 시 로그인 화면 대신 창 닫기 확인 흐름을 사용한다. 잘못된 ID·빈 상세·조회 실패는 로그와 `journal.entry.view.load.failure`, 팝업 차단은 호출 화면의 `common.error.popup`으로 드러낸다.
...
**관련 관계 행**: 일반 관련글은 `relationType/reason/targetId/targetContentType/targetTitle`별 행으로 표시한다. 대상 제목은 `openEntryView(targetId)`로 읽기 전용 원문을 열고, 관계 ID로 연결을 해제한다.
...
**엔트리 복사 형식**: 날짜(`stdrdDt (요일)`) + 공통 `htmlToPlainText(content)` (TinyMCE HTML을 브라우저 HTML 파서로 이름·10진수·16진수 엔티티 디코딩 후 평문 변환, 마크다운 재처리 이전 원문 기준). `content` 없을 때 `markdownContent` 폴백. (`#sortOrder` 없음.) 원문·해석은 한 몸이며 `appendReflectionsToCopyText`가 CRLF로 정규화한다. `copyEntry('full')`은 모든 target 리플렉션, `copyEntry('no-pending')`은 PENDING을 제외한 target 리플렉션을 엔트리 본문 뒤에 빈 줄 1개로 이어 붙이고, `copyEntry('body')`는 리플렉션을 붙이지 않는다(두 제외 옵션은 ▾ 드롭다운으로 항상 노출; 리플렉션 없으면 세 결과 동일). 챕터·검색·스레드 복사도 같은 3단계 범위와 경계 규칙을 공유한다.
...
**데이터·표시**: target 엔트리의 `reflectionList`(`JournalEntryDto`) 한 건을 target 엔트리 본문과 태그 사이에 슬림 임베드한다. 헤더·제목·마커 없이 본문·댓글(읽기)만 흐르게 표시해 target 엔트리와 이어지는 글처럼 붙이고, 옅은 1px 좌측선이 "이 엔트리에 매달린 해석"임을 알리는 유일한 신호다. 본문(`.journal-content`) 글자 크기는 일기 `JournalEntryItem`과 같다. 접힘/펼침은 `forceCollapsedSignal` prop(`"expand"|"collapse"|null`, string — Vue 3 Boolean casting 우회)과 자체 `localCollapsedOverride`로 제어한다. 접힘 우선순위: 로컬 토글 > signal > lifecycle(RESOLVED/PENDING) 자동 > 일자 aside 리플렉션 기본 접힘 모드 > 서버 COLLAPSED. 모드 ON이면 로컬·signal·lifecycle 자동 접힘 미해당 시 접힘으로 시작하고, 모드 OFF이면 서버 COLLAPSED만 기본 분기다. `JournalDayLayout` provide 범위에서만 모드가 적용되며 검색·스레드는 OFF 계약이다. 토글 전환 시 `localCollapsedOverride`를 null로 리셋한다. 부모 엔트리를 사용자가 직접 펼치면(`localCollapsedOverride=false`) signal=`"expand"` → lifecycle 무시, 같이 펼쳐진다. signal 변경 시 `localCollapsedOverride`를 null로 초기화. 등록 시 기본 lifecycle은 PENDING이라 접힌 채 시작한다. 신규 등록 성공 직후에는 `reflectionCreatedCollapseId`로 해당 임베드만 일회성 로컬 접힘(`localCollapsedOverride=true`)을 심어, 챕터 일회성 펼침·부모 `forceCollapsedSignal="expand"`보다 우선한다. 수정 저장에는 적용하지 않으며 서버 상태는 바꾸지 않는다. 엔트리 접힘 시 `v-if="!isCollapsed"`로 DOM에서 제거된다. 엔트리를 다시 펼치면 `JournalEntryItem`이 `reinitMetronicAfterDom()`으로 임베드 ⋯(KTMenu)를 재바인딩한다. 신규 등록 부분 갱신(`patchEntryReflections`)은 `fetchDays()` 재바인딩을 타지 않으므로, 새로 렌더된 임베드 ⋯(KTMenu)를 `patchEntryReflections` 안에서 `reinitMetronicAfterDom()`으로 직접 재바인딩한다. RESOLVED는 일기 완료와 같은 초록 배경 음영과 옅은 좌측선 색으로 신호한다(`.journal-reflection-embed[data-resolved="Y"]`). 임베드 목록은 같은 대상 아래 `sortOrder` 오름차순이며, 접힌 행은 `#n 제목 (collapsed)` 형태로 순번과 (제목이 있으면) `reflection.title` 을 같은 폰트(`text-muted fs-8 fst-italic`)로 표시해 접힌 상태에서도 어떤 해석인지 식별 가능하게 한다. 임베드는 flex `[본문 | 우측 액션]`(`align-items:flex-start`)이며, 댓글·복사·⋯ 액션을 target 엔트리 액션과 같은 오른쪽 열(min-width 80px, `.journal-reflection-embed__actions`)에 정렬한다. ⋯ 메뉴의 새 창 보기는 쓰기 가능 여부와 무관하게 `openJournalEntryViewPopup(reflection.id)`로 §22-4c 읽기 전용 popup을 열며, 수정·이력·라이프사이클·상태·삭제는 기존 쓰기 가드를 따른다. 리플렉션 자체 접힘 시에는 임베드를 `align-items:stretch`, 본문(`.journal-reflection-content`)을 `d-flex flex-column justify-content-center` 로 전환해 `(collapsed)` 를 임베드 세로 중앙에 두고, 액션은 자체 `align-items:flex-start` 로 상단에 유지한다(엔트리 접힘 제목과 동형).

**상태 표현**: 루트의 `data-resolved`·`data-imprtc`·`data-refrnc`는 일기 엔트리의 `$journal-paired-states` 팔레트와 연결된다. RESOLVED=초록·중요=빨강·참조=노랑 배경 음영과 좌측선을 표시하며, 둘 이상의 상태는 각 상태선을 함께 유지한다.

**액션 UI**: Reflection 은 embed 전용(1급 행 없음)이므로 모든 관리 액션은 임베드가 담당한다. 바깥은 댓글 등록·복사·⋯(Metronic)이고, ⋯ 메뉴는 수정·「전체 (( ))」·이력·라이프사이클·상태(중요/참조)·삭제를 엔트리 메뉴와 같은 구조로 제공한다. Reflection은 대칭 `related_content` 관계에 참여하지 않으므로 관련글 추가 액션을 제공하지 않는다(`REFLECTION_ONE_TYPE.md` §6). 「전체 (( ))」(`wrapEntireNoti`)는 저장 원문 `content`의 각 `<p>`/`<li>`에 Markdown `((...))`(md-text-noti) 마커를 멱등 적용하고, 변경 시에만 Reflection modify API로 저장한 뒤 `refreshJournalEntryHostForRoute`로 호스트를 갱신한다. Markdown 수평선인 하이픈 3개 이상 단독 블록은 감싸지 않으며, 기존 실행으로 생성된 `((---))` 형태는 다음 실행에서 `---`로 복구한다. 이미 전체가 감싸진 본문은 API를 호출하지 않는다. 인라인 HTML이 있는 블록은 평문으로 평탄화한 뒤 감싼다. Reflection→Reflection 중첩 등록은 제공하지 않는다(`REFLECTION_ONE_TYPE.md` §3.1). 일기·꿈·노트를 target으로 둔 Reflection에는 「스레드에 추가」를 두지 않는다(`isPrimaryContentTargetedReflection`, `REFLECTION_ONE_TYPE.md` §5). 접기(COLLAPSED)는 임베드가 본문을 항상 보이므로 메뉴에 두지 않는다. Reflection은 완결축 밖이므로 쓰기 가드는 대상 일자 `owner_id` 파생 `isOwnedBy === true`이다. `isCreatedBy`는 감사 작성자 일치다. 삭제 확인·실패 fallback은 `journal.reflection.delete.*`를 쓴다. 액션 성공 후 `refreshJournalEntryHostForRoute`로 호스트를 갱신한다.
...
- 결과 전체 복사 split 버튼 — 주 버튼(`bi-copy`)은 `copyAll('full')`(모든 해석 포함), 주 버튼 옆 ▾ 캐럿(`bi-caret-down-fill`)이 여는 `data-kt-menu` 드롭다운은 「보류 해석 제외 복사」 `copyAll('no-pending')`와 「본문만 복사」 `copyAll('body')`를 제공한다. `appendReflectionsToCopyText`가 각 결과 엔트리 본문 뒤의 target 리플렉션을 모드별로 골라 CRLF 빈 줄로 잇는다. 버튼은 `!canCopyResults`면 비활성이고, 세 모드는 리플렉션 유무와 무관하게 항상 노출한다
...
**전체 복사·다운로드 (양 표면 공통)**: 상세 모달 헤더와 독립 페이지 card-toolbar에 `store.detailModel?.id`가 있을 때 「복사」(`bi-copy`)·「다운로드」(`bi-download`) split 버튼을 둔다. 복사는 클라이언트 `journalThreadExport.copyThreadDetail`가 스레드 제목을 머리행으로, 소속 엔트리를 검색 전체 복사(copyAll)와 동일한 `날짜(요일)`·`#순번`·본문 평문 포맷으로 클립보드에 쓴다. 주 버튼 `full`은 모든 해석 포함, ▾ 메뉴의 `no-pending`은 보류 해석 제외, `body`는 본문만이며, `appendReflectionsToCopyText`가 target 리플렉션 사이에 CRLF 빈 줄 1개를 보장한다. 성공 알림은 모드별 locale 키를 사용하고 리플렉션이 없으면 공용 성공 문구로 수렴한다. 다운로드는 `downloadThreadDetail`가 `GET /api/journal/threads/{id}/export?includeReflection=`로 이동해 `thread_{id}_@yyyyMMdd.txt` 첨부를 받는다. 서버 텍스트는 `JournalThreadExportService.buildTxt`가 챕터/엔트리 내보내기와 동일한 배너·엔트리 블록 계약(`thread: 제목` + 선택 시 `prefix: 말머리` + 소속 엔트리)으로 생성하며 복사와 배너 유무만 다르다. 소유권은 서버 `getEntriesByThread`가 검증한다. 복사/다운로드 로직은 `features/journal/utils/journalThreadExport.ts` 단일 util을 두 표면이 공유하고, 표시 데이터를 바꾸지 않고 `detailModel`·`detailEntries` SSOT만 읽는다.

**스레드 본문 이력 (양 표면 공통)**: `JournalThreadEntity`는 `HistoryEmbed`를 보유하고 DTO는 `history.historyTriggeredAt`을 목록·상세에 전달한다. 공통 attachable 수정 흐름은 본문이 실제로 바뀐 경우에만 수정 전 본문을 `history` 테이블의 `JOURNAL_THREAD` 스냅샷으로 남긴다. 제목·Prefix·소속·라이프사이클 변경은 이력 대상이 아니며 복원도 본문만 교체해 나머지 스레드 필드를 유지한다. 상세 모달·독립 상세 페이지와 목록 ⋯ 메뉴는 이력 시각이 있을 때 「히스토리」를 활성화해 `useAttachableModalStore.openHistory("JOURNAL_THREAD", id)`를 호출한다. `JournalThreadHistoryStrategy`가 현재 사용자 소유 조회와 복원을 `JournalThreadService`에 위임한다. 복원·삭제 성공 후 스레드 route의 `JournalThreadLayout`은 열린 상세와 현재 목록을 재조회하고, 주간·월간·일간·검색 레이아웃은 공통 호스트 갱신으로 열린 상세와 배경을 맞춘다. 연간 결산의 기간 스레드 요약에서 연 문맥형 상세는 `JournalAnnualLayout`이 이력 모달을 마운트하고 성공 후 열린 상세를 재조회한다.
