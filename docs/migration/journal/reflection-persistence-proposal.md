...
> **범위**: 영속 target 설계와 현재→target 델타. 인프라 실측(embedding·entity)은 **완료**(§3). 구현(un-merge·삭제 정책)은 후속 SAVEPOINT.
...
- 남은 것은 검증이 아니라 **구현**: un-merge SQL과 대상 삭제 정책(§5).
...
도메인 정책([reflection-domain-model.md](reflection-domain-model.md) §5)은 **Reference→Block(재귀)** 이다. 런타임에서 `JournalEntryService.preDelete`·`JournalReflectionService.preDelete`·`JournalChapterService.preDelete`가 참조 Reflection 존재 시 삭제를 거부한다(DB FK 없음 → 서비스 레벨 Block). nullify→독립화 훅은 제거됐다. 여러 도메인의 관계 생명주기를 일반화하는 논의는 [관계 생명주기 의미론 아이디어](../../ideas/relationship-lifecycle-semantics.md)로 분리한다.
