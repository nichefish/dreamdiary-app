package io.nicheblog.dreamdiary.feature.jrnl.intrpt.repository.jpa;
...
public interface JrnlIntrptRepository
     ...
     *
     * @param jrnlDreamId 조회할 항목 번호
     * @return {@link Optional} -- 해당 일자에서 일기의 마지막 인덱스
     */
    @Query("SELECT MAX(intrpt.idx) " +
            "FROM JrnlIntrptEntity intrpt " +
            "WHERE intrpt.jrnlDream.id = :jrnlDreamId")
    Optional<Integer> findLastIndexByJrnlDay(final @Param("jrnlDreamId") Integer jrnlDreamId);
}
