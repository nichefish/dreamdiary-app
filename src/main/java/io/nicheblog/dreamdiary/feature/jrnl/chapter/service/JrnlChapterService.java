package io.nicheblog.dreamdiary.feature.jrnl.chapter.service;
...
import io.nicheblog.dreamdiary.feature.jrnl.chapter.repository.jpa.JrnlChapterRepository;
import io.nicheblog.dreamdiary.feature.jrnl.chapter.repository.mybatis.JrnlChapterMapper;
import io.nicheblog.dreamdiary.feature.jrnl.chapter.spec.JrnlChapterSpec;
 ...
 * @author nichefish
 */
@Service("jrnlChapterService")
@RequiredArgsConstructor
@Log4j2
public class JrnlChapterService
        implements BaseClsfService<JrnlChapterDto, JrnlChapterDto, Integer, JrnlChapterEntity> {

    /** 동일 일자 내 첫 항목 등록 시 기본 카테고리 코드 */
    private static final String FIRST_CHAPTER_CTGR_CD = "SUMMARY";

    @Getter
    private final JrnlChapterRepository repository;
    @Getter
    private final JrnlChapterSpec spec;
    @Getter
    private final JrnlChapterMapstruct mapstruct;

    public JrnlChapterMapstruct getReadMapstruct() {
        return this.mapstruct;
    }
    public JrnlChapterMapstruct getWriteMapstruct() {
        return this.mapstruct;
    }

    private final JrnlChapterMapper jrnlChapterMapper;
    private final JrnlCacheEvictWorker jrnlCacheEvictWorker;

    private final ApplicationContext context;
    private JrnlChapterService getSelf() {
     ...
     */
    @Override
    public void postRegist(final JrnlChapterDto updatedDto) throws Exception {
        // 관련 캐시 삭제
        jrnlCacheEvictWorker.evictAfterCommit(JrnlCacheEvictParam.of(updatedDto), ContentType.JRNL_CHAPTER);
    }
     ...
     */
    @Override
    public void postModify(final JrnlChapterDto postDto, final JrnlChapterDto updatedDto) throws Exception {
        // 인덱스 재조정
    ...
    @Override
    public void postDelete(final JrnlChapterDto deletedDto) throws Exception {
        // 인덱스 재조정
        this.getSelf().reorderIdx(deletedDto);

        // 관련 캐시 삭제
        jrnlCacheEvictWorker.evictAfterCommit(JrnlCacheEvictParam.of(deletedDto), ContentType.JRNL_CHAPTER);
    }
    ...
    public void normalize(final Integer jrnlDayNo) {
        ...
        int idx = 1;
        for (final JrnlChapterDto e : list) {
            e.setIdx(idx++);
        }

        jrnlChapterMapper.batchUpdateIdx(list);
    }
