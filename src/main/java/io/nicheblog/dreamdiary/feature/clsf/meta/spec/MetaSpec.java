package io.nicheblog.dreamdiary.feature.clsf.meta.spec;
...
public class MetaSpec
        implements BaseSpec<MetaEntity> {
    ...
    public List<Predicate> getPredicateWithParams(
        ...
        predicate.add(builder.equal(metaContentJoin.get("regstrId"), AuthUtils.getLgnUsernameOrDefault()));     // 등록자 ID 기준으로 조회
        ...
        for (final String key : searchParamMap.keySet()) {
            ...
            switch (key) {
                case "contentType":
                    predicate.add(builder.equal(metaContentJoin.get("refContentType"), value));
                default:
    ...
    /**
     * preset된 특정 검색 조건 목록을 반환한다. (메타-컨텐츠와 연관관계 없는 메타 조회)
     *
     * @return {@link Specification} -- 검색 조건에 따른 Specification 객체
     */
    public Specification<MetaEntity> getNoRefMetas() {
    ...
    /** 
     * 컨텐츠 타입에 맞는 메타 목록 조회 조건을 반환한다.
     *
     * @param contentType - 조회할 컨텐츠 타입
     * @return {@link Specification} -- 검색 조건에 따른 Specification 객체
     */
    public Specification<MetaEntity> getContentSpecificMeta(final String contentType) {
