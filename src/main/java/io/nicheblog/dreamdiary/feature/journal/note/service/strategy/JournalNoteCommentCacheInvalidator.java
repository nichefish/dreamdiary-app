...
public class JournalNoteCommentCacheInvalidator
    ...
    public boolean supports(final ContentType refContentType) {
        return ContentType.JOURNAL_NOTE.equals(refContentType);
    }
    ...
    public void invalidate(final Integer refId) throws Exception {
        final JournalNoteDto journalNoteDto = journalNoteService.getDtlDto(refId);
        final JournalCacheEvictParam param = JournalCacheEvictParam.of(journalNoteDto);
        journalCacheEvictWorker.evictAfterCommit(param, ContentType.JOURNAL_NOTE);
    }
}
