...
public class JournalIntrptSmpEntity {

    /** 필수: 컨텐츠 타입 */
    @Builder.Default
    private static final ContentType CONTENT_TYPE = ContentType.JOURNAL_INTRPT;
    ...
    /** 저널 꿈 번호  */
    @Column(name = "journal_dream_id")
    @Comment("저널 꿈 번호")
    private Integer journalDreamId;

    /** 저널 꿈 정보 */
    @ManyToOne(fetch = FetchType.EAGER)
    @JoinColumn(name = "journal_dream_id", referencedColumnName = "id", insertable = false, updatable = false)
    @Fetch(FetchMode.JOIN)
    @NotFound(action = NotFoundAction.IGNORE)
    @Comment("저널 꿈 정보")
    private JournalDreamSmpEntity journalDream;
}
