...
@AllArgsConstructor
@Where(clause = "ref_content_type='JOURNAL_INTRPT' AND deleted_at IS NULL")
@SQLDelete(sql = "UPDATE tag_content SET deleted_at = NOW() WHERE id = ?")
public class JournalIntrptTagContentEntity
    ...
    @NotFound(action = NotFoundAction.IGNORE)
    @Comment("저널 해석 정보")
    private JournalIntrptSmpEntity journalIntrpt;
