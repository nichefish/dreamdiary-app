...
public class JournalDiaryService
        implements BaseAttachableService<JournalDiaryPostDto, JournalDiaryDto, Integer, JournalDiaryEntity>, BaseMultipartWritableService<JournalDiaryPostDto, JournalDiaryDto, Integer, JournalDiaryEntity> {
    ...
    public void preRegist(final JournalDiaryPostDto registDto) throws Exception {
        assertDiaryChapterForDiary(registDto.getJournalChapterId());
        // 정렬 순서 처리
    ...
    public void preModify(final JournalDiaryPostDto modifyDto, final JournalDiaryEntity modifyEntity) throws Exception {
        ...
        assertDiaryChapterForDiary(modifyDto.getJournalChapterId());
    ...
    public JournalDiaryDto updtContent(
        ...
        final JournalDiaryEntity historySnapshot = BaseAttachableHistoryHelper.isHistoryModule(restoreEntity)
    ...
    public void postDelete(final JournalDiaryDto deletedDto) throws Exception {
        ...
        // 관련 캐시 삭제
        // 관련글 soft-delete
        relatedContentService.deleteAllByRef(new BaseAttachableKey(deletedDto.getId(), ContentType.JOURNAL_DIARY), deletedDto.getCreatedBy());
    ...
    private void assertDiaryChapterForDiary(final Integer journalChapterId) {
        ...
        if (chapter.getChapterType() != ChapterType.DIARY) {
            throw new BusinessException("msg.journal.diary.diary-chapter-only");
        }
    }
}
