package io.nicheblog.dreamdiary.feature.journal.note.spec;
 ...
 * JournalNoteSpec
 * <pre>
 *  저널 노트 목록 검색인자 세팅 Specification.
 * </pre>
 *
 * @author nichefish
 */
@Component
@Log4j2
public class JournalNoteSpec
        implements BaseAttachableSpec<JournalNoteEntity> {
    ...
    public void postQuery(
            final Root<JournalNoteEntity> root,
        ...
        final Join<JournalNoteEntity, JournalChapterSmpEntity> journalChapterJoin = root.join("journalChapter", JoinType.INNER);
        ...
        for (final String key : searchParamMap.keySet()) {
            ...
            switch (key) {
                ...
                case "journalChapterId":
                    // 챕터 id alias 검색
                    predicate.add(builder.equal(journalChapterJoin.get("id"), value));
                    continue;
                case "searchKeywords":
                    // 내용 like 검색
                ...
                case "tagId":
                    // 특정 태그된 노트만 검색
                    final Join<JournalNoteEntity, TagEmbed> tagJoin = root.join("tag", JoinType.INNER);
                    ...
                    predicate.add(builder.equal(tagContentJoin.get("refContentType"), ContentType.JOURNAL_NOTE.key));
                    continue;
                case "tagIds":
                    // 태그 exists 검색
                    if (!(value instanceof List<?> rawList)) continue;
                    ...
                    sub.where(
                        builder.and(
                            ...
                            builder.equal(subRoot.get("refContentType"), ContentType.JOURNAL_NOTE.key),
