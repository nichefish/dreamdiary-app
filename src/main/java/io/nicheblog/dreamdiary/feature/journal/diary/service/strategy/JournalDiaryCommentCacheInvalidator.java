...
public class JournalDiaryCommentCacheInvalidator
    ...
    public boolean supports(final ContentType refContentType) {
        return ContentType.JOURNAL_DIARY.equals(refContentType);
    }
    ...
    public void invalidate(final Integer refId) throws Exception {
        final JournalDiaryDto journalDiaryDto = journalDiaryService.getDtlDto(refId);
        final JournalCacheEvictParam param = JournalCacheEvictParam.of(journalDiaryDto);
        journalCacheEvictWorker.evictAfterCommit(param, ContentType.JOURNAL_DIARY);
    }
}
