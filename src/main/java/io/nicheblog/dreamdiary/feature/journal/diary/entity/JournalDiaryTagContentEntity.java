...
@AllArgsConstructor
@Where(clause = "ref_content_type='JOURNAL_DIARY' AND deleted_at IS NULL")
@SQLDelete(sql = "UPDATE tag_content SET deleted_at = NOW() WHERE id = ?")
public class JournalDiaryTagContentEntity
    ...
    @NotFound(action = NotFoundAction.IGNORE)
    @Comment("저널 일기 정보")
    private JournalDiarySmpEntity journalDiary;
}
