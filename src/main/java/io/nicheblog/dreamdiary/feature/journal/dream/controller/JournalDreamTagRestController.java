...
public class JournalDreamTagRestController
     ...
     */
    @GetMapping(Url.JOURNAL_DREAM_TAG_CTGR_MAP)
    @Secured({Constant.ROLE_USER, Constant.ROLE_MNGR})
    @ResponseBody
    public ResponseEntity<AjaxResponse> journalDreamTagCtgrMapAjax() throws Exception {
        final Map<String, List<String>> tagCtgrMap = myJournalDreamTagService.getMyTagCtgrMap();
     ...
     * @return {@link ResponseEntity} -- 처리 결과와 메시지
     */
    @GetMapping(Url.JOURNAL_DREAM_TAGS)
    @Secured({Constant.ROLE_USER, Constant.ROLE_MNGR})
    @ResponseBody
    public ResponseEntity<AjaxResponse> tagListAjax(
        ...
        if (searchParam.hasWeekStartDt()) {
            tagList = myJournalDreamTagService.getMyWeeklySizedListDto(searchParam.getWeekStartDt());
        } else if (searchParam.hasYyMnth()) {
            tagList = myJournalDreamTagService.getMyDreamSizedListDto(searchParam.getYy(), searchParam.getMnth());
        } else {
            tagList = myJournalDreamTagService.getMyTagList();
        }
     ...
     * @return {@link ResponseEntity} -- 처리 결과와 메시지
     */
    @GetMapping(Url.JOURNAL_DREAM_TAG_GROUP_LIST_AJAX)
    @Secured({Constant.ROLE_USER, Constant.ROLE_MNGR})
    @ResponseBody
    public ResponseEntity<AjaxResponse> tagGroupListAjax(
        ...
        final Map<String, List<TagDto>> tagGroupMap = searchParam.hasWeekStartDt()
                ? myJournalDreamTagService.getMyWeeklySizedGroupListDto(searchParam.getWeekStartDt())
                : myJournalDreamTagService.getMyDreamSizedGroupListDto(searchParam.getYy(), searchParam.getMnth());
        final boolean isSuccess = true;
        final String rsltMsg = MessageUtils.RSLT_SUCCESS;
