...
/**
 * JournalIntrptCommentCacheInvalidator
 * <pre>
 *  댓글 캐시 무효화 방식 정의.
 * </pre>
 *
...
public class JournalIntrptCommentCacheInvalidator
    ...
    public boolean supports(final ContentType refContentType) {
        return ContentType.JOURNAL_INTRPT.equals(refContentType);
    }
    ...
    public void invalidate(final Integer refId) throws Exception {
        final JournalIntrptDto journalIntrptDto = journalIntrptService.getDtlDto(refId);
        final JournalCacheEvictParam param = JournalCacheEvictParam.of(journalIntrptDto);
        journalCacheEvictWorker.evictAfterCommit(param, ContentType.JOURNAL_INTRPT);
    }
}
