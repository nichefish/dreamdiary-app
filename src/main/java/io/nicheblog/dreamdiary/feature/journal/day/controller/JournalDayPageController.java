...
public class JournalDayPageController
    ...
    @Getter
    private final String baseUrl = Url.JOURNAL_DAY_WEEKLY;
    @Getter
     ...
     */
    @GetMapping(Url.JOURNAL_DAY_MONTHLY)
    @Secured({Constant.ROLE_USER, Constant.ROLE_MNGR})
    public String journalDayMonthlyPage(
            final @ModelAttribute("searchParam") JournalDaySearchParam searchParam,
        ...
        if (searchParam.getYy() != null && searchParam.getMnth() != null) {
            cdLookupService.setCdListToModel(Code.JOURNAL_CHAPTER_CTGR_CD, model);
            cdLookupService.setCdListToModel(Code.TEXT_CLASS_CD, model);
            return "/view/feature/journal/day/journal_day_monthly";
        }
        ...
        return "redirect:" + Url.JOURNAL_DAY_MONTHLY + "?yy=" + defaultYy + "&mnth=" + defaultMnth;
    }
     ...
     * @return {@link String} -- 화면 뷰 경로
     */
    @GetMapping(Url.JOURNAL_DAY_WEEKLY)
    @Secured({Constant.ROLE_USER, Constant.ROLE_MNGR})
    public String journalDayWeeklyPage(
            final @ModelAttribute("searchParam") JournalDaySearchParam searchParam,
            final ModelMap model
    ) throws Exception {
        ...
        if (StringUtils.isNotBlank(searchParam.getStdrdDt())) {
            ...
            cdLookupService.setCdListToModel(Code.JOURNAL_CHAPTER_CTGR_CD, model);
            cdLookupService.setCdListToModel(Code.TEXT_CLASS_CD, model);
            return "/view/feature/journal/day/journal_day_weekly";
        }

        final String today = DateUtils.getCurrDateStr(DatePtn.DATE);
        return "redirect:" + Url.JOURNAL_DAY_WEEKLY + "?stdrdDt=" + today;
    }
     ...
     * @return {@link String} -- 화면 뷰 경로
     */
    @GetMapping(Url.JOURNAL_DAY_DAILY_VIEW_TODAY)
    public String journalDayViewTodayPage() throws Exception {
        final String today = DateUtils.getCurrDateStr(DatePtn.DATE);
     ...
     * @return {@link String} -- 화면 뷰 경로
     */
    @GetMapping(Url.JOURNAL_DAY_DAILY_VIEW)
    @Secured({Constant.ROLE_USER, Constant.ROLE_MNGR})
    public String journalDayViewDailyPage(
