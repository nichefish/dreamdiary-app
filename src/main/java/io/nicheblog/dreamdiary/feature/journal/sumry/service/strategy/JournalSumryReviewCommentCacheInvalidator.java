...
/**
 * JournalSumryReviewCommentCacheInvalidator
 * <pre>
 *  댓글 캐시 무효화 방식 정의.
 * </pre>
 *
...
public class JournalSumryReviewCommentCacheInvalidator
    ...
    public boolean supports(final ContentType refContentType) {
        return ContentType.JOURNAL_SUMRY_REVIEW.equals(refContentType);
    }
    ...
    public void invalidate(final Integer refId) throws Exception {
        final JournalSumryReviewDto journalSumryReviewDto = journalSumryReviewService.getDtlDto(refId);
        final JournalCacheEvictParam param = JournalCacheEvictParam.of(journalSumryReviewDto);
        journalCacheEvictWorker.evictAfterCommit(param, ContentType.JOURNAL_SUMRY_REVIEW);
    }
}
