...
@Log4j2
class JournalDreamRepositoryTest {
    ...
    void setUp() throws Exception {
        ...
        journalDreamEntity = JournalDreamEntityTestFactory.createWithJournalDt("2000-01-01");
    }
    ...
    public void testRegist() throws Exception {
        ...
        // When::
        final JournalDreamEntity registered = journalDreamRepository.save(journalDreamEntity);
        Integer key = registered.getId();
        final JournalDreamEntity retrieved = journalDreamRepository.findById(key).orElseThrow(() -> new EntityNotFoundException(MessageUtils.getMessage("exception.EntityNotFoundException.registered")));
    ...
    public void testModify() throws Exception {
        // Given::
        JournalDreamEntity registered = journalDreamRepository.save(journalDreamEntity);
        Integer key = registered.getId();

        // When::
        JournalDreamEntity toModify = journalDreamRepository.findById(key).orElseThrow(() -> new EntityNotFoundException(MessageUtils.getMessage("exception.EntityNotFoundException.to-modify")));
        toModify.setCn("modified");
        JournalDreamEntity modified = journalDreamRepository.save(toModify);
    ...
    public void testDelete() throws Exception {
        ...
        // When::
        final JournalDreamEntity toDelete = journalDreamRepository.findById(key).orElseThrow(() -> new EntityNotFoundException(MessageUtils.getMessage("exception.EntityNotFoundException.to-delete")));
        journalDreamRepository.delete(toDelete);

        final JournalDreamEntity retrieved = journalDreamRepository.findById(key).orElse(null);
